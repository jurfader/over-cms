    ...
    <>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .glass { background: rgba(0,0,0,0.02); backdrop-filter: blur(12px); border: 1px solid rgba(0,0,0,0.08); }
        ...
        .btn-outline { background: transparent; color: var(--color-fg); border: 1.5px solid rgba(0,0,0,0.15); }
      ...
      {html ? (
        ...
        <div style={{
          ...
          Kliknij moduł w panelu po lewej aby dodać
        </div>
      )}
    </>
  )
  ...
  return blocks.map((block) => {
    ...
    switch (type) {
      ...
      case 'row': {
        ...
        return `<div ${attrs} class="${mergeClass('', style)}" style="${mergeStyle(`display:grid;grid-template-columns:${gridCols};gap:1.5rem;min-height:80px`, style)}">${colsHtml || `<div style="min-height:60px;border:1px dashed rgba(0,0,0,0.1);border-radius:8px;"></div>`}</div>`
      }
      ...
      case 'paragraph':
        return `<p ${attrs} data-field="text" class="${mergeClass('', style)}" style="${mergeStyle('color:var(--color-muted);line-height:1.75;font-size:1rem', style)}">${esc(data.text) || '&nbsp;'}</p>`

      case 'quote':
        return `<blockquote ${attrs} data-field="text" class="${mergeClass('', style)}" style="${mergeStyle('border-left:3px solid var(--color-primary);padding-left:1.5rem;font-style:italic;color:rgba(0,0,0,0.65)', style)}">${esc(data.text)}</blockquote>`

      case 'code':
        return `<pre ${attrs} class="${mergeClass('', style)}" style="${mergeStyle('background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);border-radius:var(--radius);padding:1rem;overflow-x:auto;font-family:monospace;font-size:0.875rem;color:var(--color-muted)', style)}"><code>${esc(data.code)}</code></pre>`
      ...
      case 'image': {
        ...
        const imgTag = src
          ...
          : `<div style="background:rgba(0,0,0,0.03);border:1px dashed rgba(0,0,0,0.15);border-radius:var(--radius);height:200px;display:flex;align-items:center;justify-content:center;color:rgba(0,0,0,0.2);font-size:0.875rem;">Obraz</div>`
      ...
      case 'gallery': {
        ...
        const imgsHtml = images.length > 0
          ...
          : '<div style="color:rgba(0,0,0,0.2);font-size:0.875rem;">Galeria (brak obrazow)</div>'
      ...
      case 'video': {
        ...
        if (!url) {
          return `<div ${attrs} class="${mergeClass('', style)}" style="${mergeStyle('background:rgba(0,0,0,0.03);border:1px dashed rgba(0,0,0,0.15);border-radius:var(--radius);height:300px;display:flex;align-items:center;justify-content:center;color:rgba(0,0,0,0.2)', style)}">Video</div>`
        }
      ...
      // ── Layout ─────────────────────────────────────────────────────
      case 'divider':
        return `<hr ${attrs} class="${mergeClass('', style)}" style="${mergeStyle('border:none;border-top:1px solid rgba(0,0,0,0.08);margin:1rem 0', style)}"/>`
      ...
      case 'tabs': {
        ...
        return `<div ${attrs} class="${mergeClass('', style)}" style="${mergeStyle('', style)}"><div style="display:flex;border-bottom:1px solid rgba(0,0,0,0.08);gap:0;">${tabBtns}</div><div style="padding:1.5rem 0;color:var(--color-muted);">${esc(firstContent)}</div></div>`
      }
      ...
      case 'accordion': {
        ...
        const accHtml = items.map((item) =>
          `<details style="border:1px solid rgba(0,0,0,0.08);border-radius:var(--radius-sm);overflow:hidden;"><summary style="padding:1rem;cursor:pointer;font-weight:600;list-style:none;">${esc(item.question) || 'Pytanie'}</summary><div style="padding:0 1rem 1rem;color:var(--color-muted);font-size:0.875rem;">${esc(item.answer)}</div></details>`
        ).join('')
        return `<div ${attrs} class="${mergeClass('', style)}" style="${mergeStyle('display:flex;flex-direction:column;gap:0.5rem', style)}">${accHtml || '<details style="border:1px solid rgba(0,0,0,0.08);border-radius:var(--radius-sm);"><summary style="padding:1rem;cursor:pointer;">FAQ</summary></details>'}</div>`
      }

      case 'form': {
        const fields = (data.fields ?? []) as Array<{ label: string; type: string }>
        const fieldsHtml = fields.map((f) =>
          `<div style="display:flex;flex-direction:column;gap:0.25rem;"><label style="font-size:0.8125rem;font-weight:600;">${esc(f.label)}</label><input type="${esc(f.type) || 'text'}" style="background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.1);border-radius:var(--radius-sm);padding:0.625rem 0.75rem;color:var(--color-fg);font-size:0.875rem;" /></div>`
        ).join('')
      ...
      case 'slider': {
        ...
        if (!slide) {
          return `<div ${attrs} class="${mergeClass('', style)}" style="${mergeStyle('background:rgba(0,0,0,0.03);border:1px dashed rgba(0,0,0,0.15);border-radius:var(--radius-lg);padding:4rem 2rem;text-align:center;color:rgba(0,0,0,0.2)', style)}">Slider (dodaj slajdy)</div>`
        }
      ...
      case 'testimonial':
        return `<div ${attrs} class="${mergeClass('glass', style)}" style="${mergeStyle('border-radius:var(--radius-lg);padding:2rem', style)}"><blockquote style="font-size:0.9375rem;color:rgba(0,0,0,0.65);line-height:1.75;margin:0 0 1rem;">"${esc(data.text)}"</blockquote><p style="font-weight:700;">${esc(data.name)}</p><p style="font-size:0.8125rem;color:rgba(0,0,0,0.35);">${esc(data.role)}</p></div>`
      ...
      case 'pricing': {
        const featured = data.featured ? 'border:2px solid var(--color-primary);' : 'border:1px solid rgba(0,0,0,0.08);'
        const features = String(data.features ?? '').split('\n').filter(Boolean).map(f => `<li style="padding:0.375rem 0;border-bottom:1px solid rgba(0,0,0,0.04);font-size:0.875rem;color:var(--color-muted);">${esc(f)}</li>`).join('')
      ...
      case 'team': {
        const photo = data.photo ? `<img src="${esc(data.photo)}" alt="${esc(data.name)}" style="width:80px;height:80px;border-radius:50%;object-fit:cover;margin:0 auto 1rem;display:block;"/>` : `<div style="width:80px;height:80px;border-radius:50%;background:rgba(0,0,0,0.04);margin:0 auto 1rem;display:flex;align-items:center;justify-content:center;color:rgba(0,0,0,0.2);font-size:1.5rem;">&#128100;</div>`
      ...
      case 'social': {
        ...
        const linksHtml = items.map(item =>
          `<a href="${esc(item.url) || '#'}" style="display:inline-flex;align-items:center;justify-content:center;width:2.5rem;height:2.5rem;border-radius:50%;background:rgba(0,0,0,0.04);color:var(--color-muted);text-decoration:none;font-size:0.75rem;font-weight:700;text-transform:uppercase;">${esc(item.platform).slice(0, 2)}</a>`
        ).join('')
      ...
      // ── Fallback ───────────────────────────────────────────────────
      default:
        return `<div ${attrs} class="${mergeClass('', style)}" style="${mergeStyle('padding:1rem;border:1px dashed rgba(0,0,0,0.1);border-radius:8px;color:var(--color-muted);font-size:0.875rem', style)}">${esc(type)}: ${esc(data.text ?? data.title ?? data.label ?? '(modul)')}</div>`
    }
  }).join('\n')
}
