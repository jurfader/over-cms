...
function buildStyleAttr(style?: Record<string, unknown>): string {
  ...
  if (style.shadowX || style.shadowY || style.shadowBlur || style.shadowSpread) {
    ...
    const color = style.shadowColor ?? 'rgba(255,255,255,0.15)'
  ...
  return blocks.map((block) => {
    ...
    switch (type) {
      ...
      case 'pricing': {
        ...
        const features = String(data.features ?? '').split('\n').filter(Boolean).map(f => `<li style="padding:0.375rem 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:0.875rem;color:var(--color-muted);">${esc(f)}</li>`).join('')
